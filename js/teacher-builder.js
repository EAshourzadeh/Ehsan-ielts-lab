document.addEventListener("DOMContentLoaded", function () {
  ...
  let workingExam = null; // in-memory copy; only written to storage on Submit
  ...
  document.getElementById("builderExamSelect").addEventListener("change", (e) => {
  ...
  function loadExam(examId) {
    ...
    workingExam = JSON.parse(JSON.stringify(exams[examId])); // deep clone — edits stay local until Submit
  ...
  document.getElementById("btnSubmitExam").addEventListener("click", () => {
    ...
    const exams = getExams();
    exams[workingExam.id] = workingExam;
    saveExams(exams);
    hasUnsavedChanges = false;
